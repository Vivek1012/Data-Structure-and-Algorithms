// finding minimum in a range using Segment Tree
...
public class RangeMinimumQuery
{
	...
	public RangeMinimumQuery(int a[] ,int n) 
	{
...
//		}
	...
	private static int findSuminRange(int i, int j, int l, int r, int k, int n) {
		...
		return Math.min(findSuminRange(i , mid , l , r , 2*k + 1, n) , findSuminRange(mid+1 , j , l , r , 2*k + 2, n));
	...
	public int ConstructST(int arr[] , int start , int end , int pos)
	{
		...
		SegmentTree[pos] = Math.min(ConstructST(arr , start , mid , 2 * pos + 1) , ConstructST(arr , mid + 1 , end , 2 * pos + 2));
